Shader "Hellbound/Self-Illumin" {

Properties {
	_Color ("Diffuse Color", Color) = (1,1,1,1)
	_IllumColor("Illumination Color", Color) = (1, 0, 0, 1)
	_MainTex ("Base (RGB) Alpha (A)", 2D) = "white" {}
	_SpecTex ("Specular Color (RGB) Roughness (A)", 2D) = "black" {}
	_BumpMap ("Normalmap", 2D) = "bump" {}
	
	_Illum ("Illumin (RGB) Alpha (A)", 2D) = "white" {}
...
// #define LUX_AO_ON
	...
	#include "../Lux/Lux Shader/LuxCore/LuxLightingDirect.cginc"
	...
	void surf (Input IN, inout SurfaceOutputLux o) {
		...
		fixed4 illumination = tex2D(_Illum, IN.uv_MainTex) * _IllumColor;
		// Diffuse Albedo
		...
		#include "../Lux/Lux Shader/LuxCore/LuxLightingAmbient.cginc"
