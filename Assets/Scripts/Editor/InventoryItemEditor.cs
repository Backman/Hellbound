...
public class InventoryItemEditor : Editor
{
	...
	*/
	public override void OnInspectorGUI ()
	{
		...
		NGUIEditorTools.DrawSeparator();
		InventoryItem invItem = GameObject.FindGameObjectWithTag("InventoryKey").GetComponent<InventoryItem_Key>();
		
		// Database ID
		int dbID = EditorGUILayout.IntField("Database ID", 0);
		UIAtlas atlas = EditorGUILayout.ObjectField("Icon Atlas", invItem.getAtlas(), typeof(UIAtlas), false) as UIAtlas;
		...
		if(GUILayout.Button("New Item"))
		{
			NGUIEditorTools.RegisterUndo("Add Inventory Item", invItem);
		}
		...
		GUILayout.BeginHorizontal();
		{
			if(GUILayout.Button("<<")){ mIndex = Mathf.Max(0, mIndex - 1); }
			GUI.color = Color.white;
			EditorGUILayout.IntField(mIndex, GUILayout.Width(40f));
			GUILayout.Label("/ 20", GUILayout.Width(40f));
			if(GUILayout.Button(">>")){ mIndex = Mathf.Min(20, mIndex + 1); }
			GUI.color = Color.white;
		}
		GUILayout.EndHorizontal();
		NGUIEditorTools.DrawSeparator();
		...
		GUILayout.BeginHorizontal();
		{
			string itemName = EditorGUILayout.TextField("Item Name", "key");
